...
# Updates public messages table
def updatePublicMessages(user, message, time, record, signature):
    with sqlite3.connect(MESSAGE_DB) as db:
        db.execute("INSERT INTO PUBLIC_MESSAGES (USER,MESSAGE,TIME,LOGINSERVER_RECORD,SIGNATURE) VALUES (?,?,?,?,?)", (user, message, time, record, signature))
   
# Updates private messages table
def updatePrivateMessages(user, message, cameFrom, time, record, signature,public_key):
    with sqlite3.connect(MESSAGE_DB) as db:
        db.execute("INSERT INTO PRIVATE_MESSAGES (USER,MESSAGE,FROM_USER,TIME,USER_PUBLICKEY,SIGNATURE,LOGINSERVER_RECORD) VALUES (?,?,?,?,?,?,?)", (user, message, cameFrom, time,public_key,signature,record))
...
# Retrieves all private messages
...
##                                        ## 
#   DEBUGGING PURPOSES OF MESSAGE HANDLDER #
##                                        ## 
