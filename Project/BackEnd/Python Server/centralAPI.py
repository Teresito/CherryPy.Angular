...
##                                              ##
#   CENTRAL SERVER API END POINTS FOR ME TO CALL #
##                                              ##

# Ping to check if user still has a valid API key
...
# Gets login server record recent public key
...
def list_apis():
    url = HOST + "/list_apis"
    return(helper.Request(url, None, None))
# Login server public key
def loginserver_pubkey():
    url = HOST + "/loginserver_pubkey"
    return(helper.Request(url, None, None))
# RX_Broadcast to login server
